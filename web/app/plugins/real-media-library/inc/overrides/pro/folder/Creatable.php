...
trait Creatable
{
    ...
    public function isValidChildrenType($type)
    {
        ...
        $this->debug("Check if children type '{$type}' of {$this->id}... is allowed here: " . ($allowed === \true ? 'All is allowed here' : 'Only ' . \json_encode($allowed) . ' is allowed here'), __METHOD__);
    ...
    public function setParent($id, $ord = -1, $force = \false)
    {
        ...
        if ($id === $this->parent) {
            ...
            if ($parent->hasChildren($this->name)) {
                throw new FolderAlreadyExistsException($id, $this->name);
            }
        }
        ...
        if ($this->id > -1) {
            ...
            Util::getInstance()->doActionAnyParentHas($this, 'Folder/RelocatedOrMoved', [$this, $id, $this->order, $force, $oldData]);
    ...
    public static function deleted_realmedialibrary_meta($meta_ids, $object_id, $meta_key)
    {
        ...
        if ($meta_key === 'lastSubOrderBy') {
            ...
            // phpcs:disable WordPress.DB.PreparedSQL
            $wpdb->query($wpdb->prepare('UPDATE ' . Core::getInstance()->getTableName() . ' SET ord=NULL, oldCustomOrder=NULL WHERE id=%d', $object_id));
            // phpcs:enable WordPress.DB.PreparedSQL
            \delete_media_folder_meta($object_id, 'subOrderAutomatically');
        }
        if ($meta_key === 'orderby') {
            // phpcs:disable WordPress.DB.PreparedSQL
            $wpdb->query($wpdb->prepare('UPDATE ' . Core::getInstance()->getTableName('posts') . ' SET nr=NULL, oldCustomNr=NULL WHERE fid=%d', $object_id));
            $wpdb->query($wpdb->prepare('UPDATE ' . Core::getInstance()->getTableName() . ' SET contentCustomOrder=0 WHERE id=%d', $object_id));
            // phpcs:enable WordPress.DB.PreparedSQL
