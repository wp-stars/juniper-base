...
class WPCF7_HTMLFormatter
{
    ...
    public function append_text( $content )
    {
        ...
        if (empty($this->stacked_elements) 
            or $this->has_parent('p') 
            or $this->has_parent(self::p_parent_elements)
        ) {
    ...
    public function start_tag( $tag )
    {
        ...
        if (in_array($tag_name, self::p_child_elements) ) {
            if (! $this->is_inside('p') 
                and ! $this->is_inside(self::p_child_elements) 
            ...
            or in_array($tag_name, self::p_parent_elements) 
            or in_array($tag_name, self::p_nonparent_elements)
        ) {
