...
trait Sortable
{
    ...
    public function contentOrder($attachmentId, $nextId, $lastIdInView = \false)
    {
         ...
         * @since 4.0.8
         */
        $changed = \apply_filters('RML/Sortable/Ids', ['attachment' => $attachmentId, 'next' => $nextId, 'lastInView' => $lastIdInView]);
    ...
    public function getAttachmentNextTo($attachmentId)
    {
        ...
        global $wpdb;
        $query = new WP_Query(['post_status' => 'inherit', 'post_type' => 'attachment', 'rml_folder' => $this->id, 'orderby' => 'rml', 'order' => 'ASC']);
    ...
    public static function posts_clauses($pieces, $query)
    {
        ...
        $pieces['join'] .= ' LEFT JOIN ' . Core::getInstance()->getTableName('posts') . ' AS rmlorder ON rmlorder.fid=IFNULL(rmlposts.fid, 0) AND rmlorder.attachment = ' . $wpdb->posts . '.ID ';
    ...
    {
        $folderId = Filter::getInstance()->getFolder(null, \true);
        if ($folderId !== null) {
    ...
    public static function delete_all_order()
    {
        global $wpdb;
        // phpcs:disable WordPress.DB.PreparedSQL
        $wpdb->query('UPDATE ' . Core::getInstance()->getTableName('posts') . ' SET nr=null, oldCustomNr=NULL');
        $wpdb->query('UPDATE ' . Core::getInstance()->getTableName() . ' SET contentCustomOrder=0');
        // phpcs:enable WordPress.DB.PreparedSQL
        Core::getInstance()->debug('Deleted the whole order of all folders', __METHOD__);
    }
}
