		...
		<?php foreach ( $columns as $column ) : ?>
			<td class="td" style="text-align:<?php echo esc_attr( $text_align ); ?>; vertical-align: middle; font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif; word-wrap:break-word;">
				...
				<?php elseif ( 'total' === $column ) : ?>
					<?php echo wp_kses_post( apply_filters( 'storeabill_email_invoice_item_total', $document->get_formatted_price( $item->get_total() ), $item, $plain_text ) ); ?>
				<?php elseif ( 'total_net' === $column ) : ?>
					<?php echo wp_kses_post( apply_filters( 'storeabill_email_invoice_item_total_net', $document->get_formatted_price( $item->get_total_net() ), $item, $plain_text ) ); ?>
				<?php elseif ( 'subtotal' === $column ) : ?>
					<?php echo wp_kses_post( apply_filters( 'storeabill_email_invoice_item_subtotal', $document->get_formatted_price( $item->get_subtotal() ), $item, $plain_text ) ); ?>
				<?php elseif ( 'subtotal_net' === $column ) : ?>
					<?php echo wp_kses_post( apply_filters( 'storeabill_email_invoice_item_subtotal_net', $document->get_formatted_price( $item->get_subtotal_net() ), $item, $plain_text ) ); ?>
				<?php else : ?>
					<?php
					$getter = "get_$column";
