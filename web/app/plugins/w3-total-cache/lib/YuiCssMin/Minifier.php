...
class Minifier
{
    ...
    private function processAtRulesAndSelectors($css)
    {
        ...
        $css = preg_replace_callback(
            '/:first-(line|letter)(\{|,)/Si', function ($matches) {
        ...
        // @import handling
        $css = preg_replace_callback(
            $this->importRegex, function ($matches) use (&$imports) {
        ...
        // @namespace handling
        $css = preg_replace_callback(
            $this->namespaceRegex, function ($matches) use (&$namespaces) {
