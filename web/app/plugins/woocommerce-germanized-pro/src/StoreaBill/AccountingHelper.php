...
class AccountingHelper {
	...
	public static function init() {
		...
		/**
		 * Third party voucher compatibility
		 */
		add_filter( 'storeabill_woo_order_coupon_is_voucher', array( __CLASS__, 'order_coupon_is_voucher' ), 10, 3 );
		 ...
		 */
		add_filter(
			'storeabill_shortcode_get_object_data',
			array(
				__CLASS__,
				'maybe_decrypt_direct_debit_shortcode_data',
			),
			10,
			4
		);
	 ...
	 * @return string
	 */
	public static function adjust_sevdesk_tax_type( $tax_type, $invoice ) {
		if ( wc_gzd_is_small_business() && 0.0 === $invoice->get_total_tax() ) {
			$tax_type = 'ss';
		}
	...
	public static function maybe_decrypt_direct_debit_shortcode_data( $return_data, $object, $field_key, $atts ) {
		$maybe_encrypted = array(
			'direct_debit_iban',
			'direct_debit_bic',
		);

		if ( in_array( $field_key, $maybe_encrypted, true ) && ! empty( $return_data ) ) {
			...
			if ( is_a( $object, 'Vendidero\StoreaBill\Interfaces\Reference' ) ) {
				$ref_object = $object->get_object();

				if ( is_a( $ref_object, 'WC_Order' ) ) {
					if ( is_callable( array( $gateway, 'order_data_may_be_legacy_encrypted' ) ) ) {
						$is_legacy = $gateway->order_data_may_be_legacy_encrypted( $ref_object );
					}
				} elseif ( is_a( $ref_object, 'WC_Customer' ) ) {
					if ( is_callable( array( $gateway, 'customer_data_may_be_legacy_encrypted' ) ) ) {
						$is_legacy = $gateway->customer_data_may_be_legacy_encrypted( $ref_object );
					}
				}
			}
			...
			if ( $gateway && is_callable( array( $gateway, 'maybe_decrypt' ) ) ) {
				try {
					$reflection       = new \ReflectionMethod( $gateway, 'maybe_decrypt' );
					$number_of_params = $reflection->getNumberOfParameters();
				} catch ( \Exception $e ) {
					$number_of_params = 1;
				}

				if ( 2 === $number_of_params ) {
					$return_data = $gateway->maybe_decrypt( $return_data, $is_legacy );
				} else {
	...
	public static function show_incl_tax( $incl_tax ) {
		return 'yes' === get_option( 'woocommerce_prices_include_tax' ) && 'incl' === get_option( 'woocommerce_tax_display_shop' );
	}
		...
		if ( $order_item = $item->get_order_item() ) {
			if ( $order = $order_item->get_order() ) {
				if ( 'yes' === $order->get_meta( '_has_split_tax' ) ) {
					...
					/**
					 * Make sure that empty values, e.g. $taxes['total'][1223] = '' are removed.
					 */
					foreach ( $taxes as $tax_type => $tax_data ) {
						$taxes[ $tax_type ] = array_filter( $tax_data );
					}

					if ( ( isset( $taxes['total'] ) && count( $taxes['total'] ) > 1 ) || ( isset( $taxes['subtotal'] ) && count( $taxes['subtotal'] ) > 1 ) ) {
						$enable_split_tax = true;
					} else {
	...
	protected static function calculate_additional_costs_taxes_based_on_main_service() {
		$calculate_based_on_main_service = function_exists( 'wc_gzd_calculate_additional_costs_taxes_based_on_main_service' ) ? wc_gzd_calculate_additional_costs_taxes_based_on_main_service() : false;

		return $calculate_based_on_main_service;
	}
	...
	public static function maybe_round_additional_costs_tax_at_subtotal( $round_tax_at_subtotal, $order_item_type, $order ) {
		if ( in_array( $order_item_type, array( 'shipping', 'fee' ), true ) && ( self::enable_split_tax_calculation() || self::calculate_additional_costs_taxes_based_on_main_service() ) ) {
	...
	/**
	 * @param boolean $is_voucher
	 * @param \WC_Order_Item_Coupon $order_item
	 * @param Order $order
	 *
	 * @return boolean
	 */
	public static function order_coupon_is_voucher( $is_voucher, $order_item, $order ) {
		$coupon = new \WC_Coupon( $order_item->get_code() );
