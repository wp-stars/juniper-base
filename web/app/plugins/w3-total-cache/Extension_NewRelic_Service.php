...
class Extension_NewRelic_Service
{
    ...
    function verify_compatibility()
    {
        ...
        sprintf(
            __('Not supported: %s.', 'w3-total-cache'),
            $php_version 
        ) );
            ...
            try {
                ...
                if ($this->get_license_key_from_ini() && $this->get_license_key_from_account()
                    ...
                    $error['license'] = sprintf(
                        __('Configured license key does not match license key(s) in account: <br />%s <br />%s', 'w3-total-cache'),
    ...
    private function getAPI()
    {
        static $api = null;
        if (!$api ) {
            include_once W3TC_LIB_NEWRELIC_DIR . '/NewRelicAPI.php';
    ...
    public function get_slowest_page_load()
    {
        ...
        if ($metric_data ) {
            ...
            foreach ( $transactions as $transaction ) {
                $key = str_replace(
                    'EndUser/WebTransaction/WebTransaction', '',
                    $transaction->name 
                );
