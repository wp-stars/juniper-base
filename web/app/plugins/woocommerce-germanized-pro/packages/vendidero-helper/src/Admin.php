...
class Admin {
	...
	public static function multisite_standalone_check() {
		...
		$message = wp_kses_post( sprintf( _x( 'To ensure smooth updates you\'ll need to %1$sinstall the standalone vendidero helper plugin%2$s network-wide.', 'vd-helper', 'woocommerce-germanized-pro' ), '<a href="' . esc_url( $install_url ) . '">', '</a>' ) );
		$plugins = array_keys( get_plugins() );

		// Helper is installed but not activated
		if ( in_array( 'vendidero-helper/vendidero-helper.php', $plugins, true ) ) {
			$message = wp_kses_post( sprintf( _x( 'The vendidero helper needs to be %1$sactivated network-wide%2$s to ensure smooth updates for your products.', 'vd-helper', 'woocommerce-germanized-pro' ), '<a href="' . esc_url( $activate_url ) . '">', '</a>' ) );
		}
			...
			if ( is_multisite() && is_network_admin() ) {
				foreach ( $product->get_blog_ids() as $blog_id ) {
					...
					if ( ! $product->is_registered( $blog_id ) ) {
						?>
						<div class="error">
							<p><?php printf( esc_html_x( 'Your %1$s license for %2$s doesn\'t seem to be registered. Please %3$s', 'vd-helper', 'woocommerce-germanized-pro' ), '<strong>' . esc_attr( $product->Name ) . '</strong>', esc_html( $blog_info->blogname ), '<a style="margin-left: 5px;" class="button button-secondary" href="' . esc_url( $admin_url ) . '">' . esc_html_x( 'manage your licenses', 'vd-helper', 'woocommerce-germanized-pro' ) . '</a>' ); ?></p>
						</div>
						<?php
					} elseif ( $product->supports_renewals() && $product->has_expired( $blog_id ) ) {
						?>
						<div class="error">
							<p><?php printf( esc_html_x( 'Your %1$s license for %2$s has expired on %3$s. %4$s', 'vd-helper', 'woocommerce-germanized-pro' ), '<strong>' . esc_attr( $product->Name ) . '</strong>', esc_html( $blog_info->blogname ), esc_html( $product->get_expiration_date( get_option( 'date_format' ), $blog_id ) ), '<a style="margin-left: 5px;" class="button button-primary wc-gzd-button" target="_blank" href="' . esc_url( $product->get_renewal_url( $blog_id ) ) . '">' . esc_html_x( 'renew now', 'vd-helper', 'woocommerce-germanized-pro' ) . '</a>' ); ?></p>
						</div>
						<?php
					}
				}
				...
				if ( ! $product->is_registered() ) {
					?>
					<div class="error">
						<p><?php printf( esc_html_x( 'Your %1$s license doesn\'t seem to be registered. Please %2$s', 'vd-helper', 'woocommerce-germanized-pro' ), '<strong>' . esc_attr( $product->Name ) . '</strong>', '<a style="margin-left: 5px;" class="button button-secondary" href="' . esc_url( $admin_url ) . '">' . esc_html_x( 'manage your licenses', 'vd-helper', 'woocommerce-germanized-pro' ) . '</a>' ); ?></p>
					</div>
				<?php } elseif ( $product->has_expired() && $product->supports_renewals() ) { ?>
					<div class="error">
						<p><?php printf( esc_html_x( 'Your %1$s license has expired on %2$s. %3$s %4$s', 'vd-helper', 'woocommerce-germanized-pro' ), '<strong>' . esc_attr( $product->Name ) . '</strong>', esc_html( $product->get_expiration_date( get_option( 'date_format' ) ) ), '<a style="margin-left: 5px;" class="button button-primary wc-gzd-button" target="_blank" href="' . esc_url( $product->get_renewal_url() ) . '">' . esc_html_x( 'renew now', 'vd-helper', 'woocommerce-germanized-pro' ) . '</a>', '<a href="' . esc_url( wp_nonce_url( admin_url( 'admin-post.php?action=vd_refresh_license_status&product_id=' . esc_attr( $product->id ) ), 'vd-refresh-license-status' ) ) . '" class="" style="margin-left: 1em;">' . esc_html_x( 'Already renewed?', 'vd-helper', 'woocommerce-germanized-pro' ) . '</a>' ); ?></p>
					</div>
					<?php
				}
			}
	...
	public static function screen() {
		...
		<div class="vd-wrapper">
			<div class="wrap about-wrap vendidero-wrap">
				<div class="col-wrap">
					<h1><?php echo esc_html_x( 'Welcome to vendidero', 'vd-helper', 'woocommerce-germanized-pro' ); ?></h1>
					<div class="about-text vendidero-updater-about-text">
						<?php echo esc_html_x( 'Easily manage your licenses for vendidero Products and enjoy automatic updates & more.', 'vd-helper', 'woocommerce-germanized-pro' ); ?>
					</div>
				</div>
			</div>
	...
	public static function process_unregister() {
		...
		if ( isset( $products[ $file ] ) ) {
			if ( ! Package::get_api()->unregister( $products[ $file ] ) ) {
				array_push( $errors, sprintf( _x( 'Sorry, there was an error while unregistering %s', 'vd-helper', 'woocommerce-germanized-pro' ), $products[ $file ]->Name ) );
			}
		}
