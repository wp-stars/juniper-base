...
class Cdn_AdminNotes
{
    ...
    public function w3tc_notes( $notes )
    {
        ...
        if (!Cdn_Util::is_engine_mirror($cdn_engine) ) {
            ...
            if ($state->get_boolean('cdn.show_note_wp_upgraded') ) {
                $notes['cdn_wp_upgraded'] = sprintf(
                    __(
                        'Upgraded WordPress? Please %s files now to ensure proper operation. %s',
                        'w3-total-cache' 
                    ),
                    Util_Ui::button_popup(
                        'upload wp-includes', 'cdn_export',
            ...
            if ($state->get_boolean('cdn.show_note_cdn_upload') 
                ...
                if ($state->get_boolean('cdn.show_note_cdn_upload') ) {
                    $notes[] = sprintf(
                        __(
                            'Make sure to %s and upload the %s, files to the <acronym title="Content Delivery Network">CDN</acronym> to ensure proper operation. %s',
                            'w3-total-cache' 
                        ),
                        Util_Ui::button_popup(
                ...
                if ($state->get_boolean('cdn.show_note_cdn_reupload') ) {
                    $notes[] = sprintf(
                        __(
                            'Settings that affect Browser Cache settings for files hosted by the CDN have been changed. To apply the new settings %s and %s. %s',
                            'w3-total-cache' 
                        ),
                        Util_Ui::button_popup(
        ...
        if (!$state->get_boolean('cdn.hide_note_no_curl') 
            ...
            $notes[] = sprintf(
                __(
                    'The <strong>CURL PHP</strong> extension is not available. Please install it to enable S3 or CloudFront functionality. %s',
                    'w3-total-cache' 
                ),
                Util_Ui::button_hide_note2(
                    array(
            ...
            if (!$upload_info ) {
                ...
                if (empty($upload_path) ) {
                    ...
                    $errors['cdn_uploads_folder_empty'] = sprintf(
                        __(
                            'The uploads directory is not available. Default WordPress directories will be created: <strong>%s</strong>.',
                            'w3-total-cache' 
                        ),
                        $upload_path 
                    );
                ...
                if (!Util_Environment::is_wpmu() ) {
                    $errors['cdn_uploads_folder_not_found'] = sprintf(
                        __(
                            'The uploads path found in the database (%s) is inconsistent with the actual path. Please manually adjust the upload path either in miscellaneous settings or if not using a custom path %s automatically to resolve the issue.',
                            'w3-total-cache' 
                        ),
                        $upload_path,
                        Util_Ui::button_link(
                            __('update the path', 'w3-total-cache'),
                            Util_Ui::url(
                                array(
