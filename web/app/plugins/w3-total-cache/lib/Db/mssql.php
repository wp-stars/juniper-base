...
"/*/WP_I18N_DB_CONN_ERROR*/, $dbhost 
         ...
         *                         prefix is requested, then the custom users and usermeta tables
         *                         will be mapped.
         ...
         *                               {@link http://php.net/vsprintf vsprintf()}, or the first variable to substitute into
         *                               the query's placeholders if being called like {@link http://php.net/sprintf sprintf()}.
...
<p>If you're unsure what these terms mean you should probably contact your host. If you still need help you can always visit the <a href='http://wordpress.org/support/'>WordPress Support Forums</a>.</p>
"/*/WP_I18N_DB_CONN_ERROR*/, $details['db_host'] 
                        ), 'db_connect_fail' 
                    );
            }
         ...
         *                                    A format is one of '%d', '%s' (decimal number, string). If omitted, all values in $data will be treated as strings unless otherwise specified
         *                                    in wpdb::$field_types.
    ...
    function read()
    {
        if (empty($this->fields_map)) {
            if (file_exists($this->filepath)) {
                $this->fields_map = include $this->filepath;
            } else {
...
class SQL_Translations
{
    ...
    {
        if ($query == "SELECT COUNT(NULLIF(meta_value LIKE '%administrator%', FALSE) as Computed), "            . "COUNT(NULLIF(meta_value LIKE '%editor%', FALSE) as Computed), "            . "COUNT(NULLIF(meta_value LIKE '%author%', FALSE) as Computed), "            . "COUNT(NULLIF(meta_value LIKE '%contributor%', FALSE) as Computed), "            . "COUNT(NULLIF(meta_value LIKE '%subscriber%', FALSE) as Computed), "            . "COUNT(*) as Computed FROM " . $this->prefix . "usermeta WHERE meta_key LIKE '" . $this->prefix . "capabilities'"
        ) {
            $query = "SELECT
