...
class Extension_NewRelic_Plugin_Admin
{
    ...
    function run()
    {
        ...
        add_filter(
            'w3tc_extension_plugin_links_newrelic',
            array( $this, 'w3tc_extension_plugin_links' ) 
        );
        add_action(
            'w3tc_settings_page-w3tc_monitoring',
    ...
    function admin_notices()
    {
        ...
        if ($not_running ) {
            ...
            $message .= '<p>' . sprintf(
                ...
                network_admin_url('admin.php?page=w3tc_general#monitoring') 
            ) . "</p>";
            $message .= "</div></p>\n";
    ...
    public function w3tc_config_save( $config )
    {
        ...
        if ($is_filled ) {
            $monitoring_type = $config->get_string(
                array(
