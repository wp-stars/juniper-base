...
<div class="input">
    ...
    <label for="<?php echo $prefix_id; ?>_is_update"><?php printf(esc_html__('%s Fields', 'wp_all_import_plugin'), $addon->name()); ?></label>
    <!--a href="#help" class="wpallimport-help" title="<?php printf(esc_html__('If Keep %s Fields box is checked, it will keep all %s Fields, and add any new %s Fields specified in %s Fields section, as long as they do not overwrite existing fields. If \'Only keep this %s Fields\' is specified, it will only keep the specified fields.', 'wp_all_import_plugin'), $addon->name(), $addon->name(), $addon->name(), $addon->name(), $addon->name(), $addon->name()); ?>">?</a-->
    <div class="switcher-target-<?php echo $prefix_id; ?>_is_update" style="padding-left:17px;">
        <div class="input">
            ...
            <label for="<?php echo $prefix_id; ?>_logic_full_update"><?php printf(esc_html__('Update all %s Fields', 'wp_all_import_plugin'), $addon->name()); ?></label>
        </div>
        <div class="input">
            ...
            <label for="<?php echo $prefix_id; ?>_logic_only"><?php printf(esc_html__('Update only these %s Fields, leave the rest alone', 'wp_all_import_plugin'), $addon->name()); ?></label>
        ...
        <div class="input">
            ...
            <label for="<?php echo $prefix_id; ?>_logic_except"><?php printf(esc_html__('Leave these fields alone, update all other %s Fields', 'wp_all_import_plugin'), $addon->name()); ?></label>
