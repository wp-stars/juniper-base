...
class CdnEngine_Base
{
    ...
    function get_domain( $path = '' )
    {
        ...
        if ($count ) {
             ...
             * Custom host for SSL
             */
            list( $domain_http, $domain_https ) = array_map(
                'trim',
                explode(',', $domain . ',') 
            );
    ...
    function _may_gzip( $file )
    {
         ...
         */
        if (preg_match(
            '~\.(ico|js|css|xml|xsd|xsl|svg|htm|html|txt)$~i',
            $file 
        ) 
        ) {
            return true;
    ...
    {
        return preg_match(
            '~[a-z0-9\-_]+(\.include-body\.[a-z0-9]+)\.js$~',
            $path 
        );
    }
    ...
    {
        return preg_match(
            '~[a-z0-9\-_]+(\.include-footer\.[a-z0-9]+)\.js$~',
            $path 
        );
    }
