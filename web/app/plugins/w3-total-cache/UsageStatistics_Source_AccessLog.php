...
class UsageStatistics_Source_AccessLog
{
    ...
    {
        $dynamic_requests_total = Util_UsageStatistics::sum(
            $history,
            array( 'access_log', 'dynamic_count' ) 
        );
        $dynamic_timetaken_ms_total = Util_UsageStatistics::sum(
            $history,
            array( 'access_log', 'dynamic_timetaken_ms' ) 
        );
        $static_requests_total = Util_UsageStatistics::sum(
            $history,
            array( 'access_log', 'static_count' ) 
        );
        $static_timetaken_ms_total = Util_UsageStatistics::sum(
            $history,
    ...
    public function __construct( $data )
    {
        ...
        $this->line_regexp = apply_filters(
