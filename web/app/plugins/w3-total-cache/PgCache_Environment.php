...
class PgCache_Environment
{
    ...
    public function fix_on_event( $config, $event, $old_config = null )
    {
        ...
        if ($config->get_boolean('pgcache.enabled') 
            ...
            if ($old_config != null 
                && $config->get_integer('pgcache.file.gc') !=       $old_config->get_integer('pgcache.file.gc') 
            ) {
                $this->unschedule_gc();
            }
        ...
        if ($config->get_boolean('pgcache.enabled') 
            ...
            if ($old_config != null 
                && $config->get_integer('pgcache.prime.interval') !=       $old_config->get_integer('pgcache.prime.interval') 
            ) {
    ...
    */
    ...
    {
        Util_Rule::add_rules(
            $exs, Util_Rule::get_pgcache_rules_core_path(),
    ...
    private function rules_core_generate_nginx( $config )
    {
        ...
        $reject_user_agents = $config->get_array('pgcache.reject.ua');
        if ($config->get_boolean('pgcache.compatibility') ) {
            $reject_user_agents = array_merge(
