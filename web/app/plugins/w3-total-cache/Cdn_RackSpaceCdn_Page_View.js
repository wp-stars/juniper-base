jQuery(
    function ($) {
         ...
         */
        .on(
            'click', '.w3tc_cdn_rackspace_authorize', function () {
                W3tc_Lightbox.open(
                    {
                        id:'w3tc-overlay',
                        close: '',
        ...
        .on(
            'click', '.w3tc_cdn_rackspace_service_create_done', function () {
                var url = ajaxurl + '?action=w3tc_ajax&_wpnonce=' + w3tc_nonce +
        ...
        .on(
            'click', '.w3tc_cdn_rackspace_protocol', function () {
                var protocol = '';

                $('body').find('.w3tc_cdn_rackspace_protocol').each(
                    function (i) {
         ...
         */
        .on(
            'click', '.w3tc_cdn_rackspace_configure_domains', function () {
                W3tc_Lightbox.open(
                    {
                        id:'w3tc-overlay',
                        close: '',
        ...
        .on(
            'click', '.w3tc_cdn_rackspace_configure_domains_done', function () {
                var url = ajaxurl + '?action=w3tc_ajax&_wpnonce=' + w3tc_nonce +
                '&w3tc_action=cdn_rackspace_configure_domains_done';

                var v = $('.w3tc_cdn_rackspace_form').find('input').each(
                    function (i) {
        ...
        .on(
            'size_change', '#cdn_cname_add', function () {
                w3tc_rackspace_resize(W3tc_Lightbox);
            }
        )
    }
