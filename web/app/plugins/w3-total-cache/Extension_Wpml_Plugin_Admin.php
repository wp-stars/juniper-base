...
class Extension_Wpml_Plugin_Admin
{
    ...
    function w3tc_notes( $notes )
    {
        ...
        if ($config->get_boolean('pgcache.enabled') 
            ...
            if (!$state->get_boolean('wpml.hide_note_language_negotiation_type') ) {
                $notes[] = sprintf(
                    __(
                        'W3 Total Cache\'s Page caching cannot work effectively when WPML Language URL formatis "Language name added as a parameter" used. Please consider another URL format. Visit the WPML -&gt; Languages settings. %s',
                        'w3-total-cache' 
                    ),
    ...
    static public function w3tc_extensions( $extensions, $config )
    {
        ...
        'author' => 'W3 EDGE',
        'description' => __(
            'Improves page caching interoperability with WPML.',
            'w3-total-cache' 
        ),
    ...
    static public function w3tc_notes_wpml( $notes )
    {
        ...
        $notes[$extension_id] = sprintf(
            __(
                'Activating the <a href="%s">WPML</a> extension for W3 Total Cache may be helpful for your site. %s%s',
                'w3-total-cache' 
            ),
