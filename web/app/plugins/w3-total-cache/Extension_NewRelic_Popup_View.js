jQuery(
    function ($) {
        $('body')
        .on(
            'click', '.w3tcnr_configure', function () {
                W3tc_Lightbox.open(
                    {
                        id:'w3tc-overlay',
                        close: '',
        ...
        .on(
            'click', '.w3tcnr_list_applications', function () {
        ...
        .on(
            'click', '.w3tcnr_apply_configuration', function () {
                var url = ajaxurl + '?action=w3tc_ajax&_wpnonce=' + w3tc_nonce +
                '&w3tc_action=newrelic_apply_configuration';
                $('.w3tcnr_form').find('input').each(
                    function (i) {
