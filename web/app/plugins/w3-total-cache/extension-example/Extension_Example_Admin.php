...
class Extension_Example_Admin
{
    ...
    public function run()
    {
        ...
        add_action(
            'w3tc_extension_page_example', array(
            $this,
            'w3tc_extension_page'
            ) 
        );
        ...
        add_action(
            'w3tc_deactivate_extension_example', array(
    ...
    {
        if ($config->get(array( 'example', 'is_title_postfix' )) !=      $old_config->get(array( 'example', 'is_title_postfix' )) 
            || $config->get(array( 'example', 'title_postfix' )) !=      $old_config->get(array( 'example', 'title_postfix' )) 
        ) {
    ...
    public function w3tc_widget_setup()
    {
        ...
        add_meta_box(
            'example', 'example', array( $this, 'widget_content' ),
            $screen, 'normal', 'core' 
        );
    }
