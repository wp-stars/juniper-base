...
class Extension_NewRelic_Widget
{
    ...
    static public function w3tc_ajax()
    {
        ...
        add_action(
            'w3tc_ajax_newrelic_widgetdata_basic', array(
            $o, 'w3tc_ajax_newrelic_widgetdata_basic' ) 
        );
        add_action(
            'w3tc_ajax_newrelic_widgetdata_pageloads', array(
            $o, 'w3tc_ajax_newrelic_widgetdata_pageloads' ) 
        );
        add_action(
            'w3tc_ajax_newrelic_widgetdata_webtransactions', array(
            $o, 'w3tc_ajax_newrelic_widgetdata_webtransactions' ) 
        );
        add_action(
            'w3tc_ajax_newrelic_widgetdata_dbtimes', array(
    ...
    static public function admin_init_w3tc_dashboard()
    {
        ...
        $view_application != 0 );
        $monitoring_type = $o->_config->get_string(
            array(
            'newrelic', 'monitoring_type' ) 
        );
        if ($monitoring_type != 'browser' ) {
            wp_enqueue_script(
                'w3tc-widget-newrelic',
    ...
    function widget_new_relic( $widget_id, $form_inputs = array() )
    {
        ...
        $monitoring_type = $this->_config->get_string(
            array(
            'newrelic', 'monitoring_type' ) 
        );
        if ($monitoring_type == 'browser' ) {
    ...
    public function w3tc_ajax_newrelic_widgetdata_basic()
    {
        ...
        try {
            ...
            $this->_fill(
                $response, 'application_busy', $summary,
                'Application Busy' 
            );
            ...
            if ($can_use_metrics ) {
                ...
                $this->_fill_avg(
                    $response, 'enduser', $dashboard_metrics,
                    'EndUser' 
                );
                $this->_fill_avg(
                    $response, 'webtransaction', $dashboard_metrics,
                    'WebTransaction' 
                );
                $this->_fill_avg(
                    $response, 'database', $dashboard_metrics,
                    'Database' 
                );
            }
