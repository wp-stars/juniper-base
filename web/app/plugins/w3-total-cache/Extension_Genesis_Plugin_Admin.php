...
class Extension_Genesis_Plugin_Admin
{
    ...
    {
        add_action(
            'w3tc_extension_page_genesis.theme', array(
    ...
    function w3tc_errors( $errors )
    {
        ...
        if (!$c->is_extension_active_frontend('fragmentcache') ) {
            ...
            __(
                'Please enable <strong>Fragment Cache</strong> module to make sure <strong>Genesis extension</strong> works properly.',
                'w3-total-cache' 
            );
        }
    ...
    static public function w3tc_notes_genesis_theme( $notes )
    {
        ...
        $notes[$extension_id] = sprintf(
            __(
                'Activating the <a href="%s">Genesis Theme</a> extension for W3 Total Cache may be helpful for your site. <a href="%s">Click here</a> to try it. %s',
                'w3-total-cache' 
            ),
