...
class Minify_Plugin_Admin
{
    ...
    function run()
    {
        ...
        if ($this->_config->get_boolean('minify.enabled') ) {
            add_filter(
                'w3tc_usage_statistics_summary_from_history', array(
        ...
        add_action(
            'w3tc_ajax_minify_help', array(
            $this,
            'w3tc_ajax_minify_help'
            ) 
        );
        add_action(
            'w3tc_message_action_minify_help', array(
            $this,
    ...
    public function w3tc_admin_bar_menu( $menu_items )
    {
        ...
        'href' => wp_nonce_url(
            network_admin_url(
                'admin.php?page=w3tc_dashboard&amp;w3tc_message_action=minify_help' 
            ), 'w3tc' 
        )
        );
    ...
    function w3tc_notes( $notes )
    {
        ...
        if ($state_note->get_boolean('minify.show_note.need_flush') 
            ...
            $notes['minify_flush_needed'] = sprintf(
                __(
                    'The setting change(s) made either invalidate the cached data or modify the behavior of the site. %s now to provide a consistent user experience.',
                    'w3-total-cache' 
                ),
                Util_Ui::button_link(
    ...
    public function w3tc_usage_statistics_summary_from_history( $summary, $history )
    {
        ...
        if ($a['requests_visible'] ) {
            // counters
            $requests_total = Util_UsageStatistics::sum(
                $history,
                'minify_requests_total' 
            );
            $original_length_css = Util_UsageStatistics::sum(
                $history,
                'minify_original_length_css' 
            );
            $output_length_css = Util_UsageStatistics::sum(
                $history,
                'minify_output_length_css' 
            );

            $original_length_js = Util_UsageStatistics::sum(
                $history,
                'minify_original_length_js' 
            );
            $output_length_js = Util_UsageStatistics::sum(
                $history,
