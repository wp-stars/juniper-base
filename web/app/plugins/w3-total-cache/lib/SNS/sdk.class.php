...
class CFRuntime
{
    ...
    public function authenticate($action, $opt = null, $domain = null, $signature_version = 2, $redirects = 0)
    {
        ...
        foreach ($headers as $header_key => $header_value)
        {
            ...
            if ($signature_version === 3 && !$this->use_ssl) {
                ...
                if (substr(strtolower($header_key), 0, 8) === 'content-' 
    ...
    public function parse_callback($response, $headers = null)
    {
        ...
        // Look for XML cues
        if ((isset($headers['content-type']) && ($headers['content-type'] === 'text/xml' || $headers['content-type'] === 'application/xml'))  // We know it's XML
