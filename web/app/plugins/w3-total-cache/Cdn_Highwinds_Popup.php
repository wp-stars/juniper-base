...
class Cdn_Highwinds_Popup
{
    static public function w3tc_ajax()
    {
        ...
        add_action(
            'w3tc_ajax_cdn_highwinds_authenticate',
            array( $o, 'w3tc_ajax_cdn_highwinds_authenticate' ) 
        );
        add_action(
            'w3tc_ajax_cdn_highwinds_select_host',
            array( $o, 'w3tc_ajax_cdn_highwinds_select_host' ) 
        );
        add_action(
            'w3tc_ajax_cdn_highwinds_configure_host',
            array( $o, 'w3tc_ajax_cdn_highwinds_configure_host' ) 
        );
        add_action(
            'w3tc_ajax_cdn_highwinds_configure_cnames_form',
            array( $o, 'w3tc_ajax_cdn_highwinds_configure_cnames_form' ) 
        );
        add_action(
            'w3tc_ajax_cdn_highwinds_configure_cnames',
    ...
    private function _create_host( $api, $host_name )
    {
        ...
        if ($origin_id == 0 ) {
            try {
                ...
                throw new \Exception(
                    'Can\'t create origin ' . $home_domain . ': ' .
                    $ex->getMessage() 
                );
            }
        }
        ...
        try {
            $configuration_response = $api->configure_scope_set(
                $host,
