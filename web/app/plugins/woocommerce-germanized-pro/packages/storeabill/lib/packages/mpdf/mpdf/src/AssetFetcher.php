...
use Vendidero\StoreaBill\Vendor\Mpdf\File\LocalContentLoaderInterface;
...
class AssetFetcher implements \Vendidero\StoreaBill\Vendor\Psr\Log\LoggerAwareInterface
{
	...
	public function fetchDataFromPath($path, $originalSrc = null)
	{
		...
		if ($wrapperChecker->hasBlacklistedStreamWrapper($path)) {
			throw new \Vendidero\StoreaBill\Vendor\Mpdf\Exception\AssetFetchingException('File contains an invalid stream. Only ' . implode(', ', $wrapperChecker->getWhitelistedStreamWrappers()) . ' streams are allowed.');
		}
	...
	public function fetchRemoteContent($path)
	{
		...
		try {
			...
			/** @var \Vendidero\StoreaBill\Vendor\Mpdf\Http\Response $response */
			...
			if ($response->getStatusCode() !== 200) {
				...
				if ($this->mpdf->debug) {
					throw new \Vendidero\StoreaBill\Vendor\Mpdf\MpdfException($message);
				}
			...
			if ($this->mpdf->debug) {
				throw new \Vendidero\StoreaBill\Vendor\Mpdf\MpdfException($message, 0, E_ERROR, null, null, $e);
			}
