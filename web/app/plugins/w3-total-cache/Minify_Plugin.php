...
class Minify_Plugin
{
    ...
    function run()
    {
        ...
        add_action(
            'w3tc_usage_statistics_of_request', array(
            $this, 'w3tc_usage_statistics_of_request' ), 10, 1 
        );
        add_filter(
            'w3tc_usage_statistics_metrics', array(
...
";
    ...
    {
        return Minify_Core::minified_url(
            $this->get_minify_manual_filename(
                $theme, $template, $location, $type 
            ) 
        );
    }
    ...
    {
        $uas = array_merge(
            $this->_config->get_array('minify.reject.ua'), array(
            W3TC_POWERED_BY
            ) 
        );
...
class _W3_MinifyHelpers
{
    ...
    public function http2_header_add( $url, $as )
    {
        ...
        // 4000 - browsercache
        $data = apply_filters(
            'w3tc_minify_http2_preload_url', array(
            'result_link' => $uri,
            'original_url' => $url
            ) 
        );
