...
class Reset
{
    ...
    {
        if ($this->isPro()) {
            \register_rest_route(Service::LEGACY_NAMESPACE, '/reset/order', ['methods' => 'DELETE', 'callback' => [$this, 'resetOrder'], 'permission_callback' => [$this, 'permission_callback']]);
        }
        \register_rest_route(Service::LEGACY_NAMESPACE, '/reset/count', ['methods' => 'DELETE', 'callback' => [$this, 'resetCount'], 'permission_callback' => [$this, 'permission_callback']]);
        \register_rest_route(Service::LEGACY_NAMESPACE, '/reset/slugs', ['methods' => 'DELETE', 'callback' => [$this, 'resetSlugs'], 'permission_callback' => [$this, 'permission_callback']]);
        \register_rest_route(Service::LEGACY_NAMESPACE, '/reset/relations', ['methods' => 'DELETE', 'callback' => [$this, 'resetRelations'], 'permission_callback' => [$this, 'permission_callback']]);
        \register_rest_route(Service::LEGACY_NAMESPACE, '/reset/folders', ['methods' => 'DELETE', 'callback' => [$this, 'resetFolders'], 'permission_callback' => [$this, 'permission_callback']]);
        \register_rest_route(Service::LEGACY_NAMESPACE, '/export', ['methods' => 'GET', 'callback' => [$this, 'export'], 'permission_callback' => [$this, 'permission_callback']]);
        if ($this->isPro()) {
            \register_rest_route(Service::LEGACY_NAMESPACE, '/import', ['methods' => 'POST', 'callback' => [$this, 'import'], 'permission_callback' => [$this, 'permission_callback']]);
            \register_rest_route(Service::LEGACY_NAMESPACE, '/import/taxonomy', ['methods' => 'POST', 'callback' => [$this, 'importTaxonomy'], 'permission_callback' => [$this, 'permission_callback']]);
            \register_rest_route(Service::LEGACY_NAMESPACE, '/import/mlf', ['methods' => 'POST', 'callback' => [$this, 'importMlf'], 'permission_callback' => [$this, 'permission_callback']]);
            \register_rest_route(Service::LEGACY_NAMESPACE, '/import/filebird', ['methods' => 'POST', 'callback' => [$this, 'importFileBird'], 'permission_callback' => [$this, 'permission_callback']]);
        }
        \register_rest_route(Service::LEGACY_NAMESPACE, '/reset/debug', ['methods' => 'DELETE', 'callback' => [$this, 'resetDebug'], 'permission_callback' => [$this, 'permission_callback']]);
        \register_rest_route(Service::LEGACY_NAMESPACE, '/notice/lite', ['methods' => 'DELETE', 'callback' => [$this, 'routeNoticeDismissLite'], 'permission_callback' => [$this, 'permission_callback_activate_plugins']]);
        \register_rest_route(Service::LEGACY_NAMESPACE, '/notice/import', ['methods' => 'DELETE', 'callback' => [$this, 'routeNoticeDismissImportTax'], 'permission_callback' => [$this, 'permission_callback_activate_plugins']]);
    }
    /**
    ...
    {
        $permit = Service::permit('manage_options');
    ...
    {
        $permit = Service::permit('activate_plugins');
        return $permit === null ? \true : $permit;
    }
    /**
     * See API docs.
    ...
    public function importMlf($request)
    {
        ...
        ExImport::getInstance()->importMlf();
        return new WP_REST_Response(['success' => \true]);
    }
    /**
     * See API docs.
    ...
    public function importFileBird($request)
    {
        ...
        ExImport::getInstance()->importFileBird();
        return new WP_REST_Response(['success' => \true]);
    }
    /**
     * See API docs.
    ...
    {
        if (($permit = Service::permit('activate_plugins')) !== null) {
            return $permit;
        }
    ...
    public function routeNoticeDismissImportTax()
    {
        ...
        ExImport::getInstance()->isImportTaxNoticeDismissed(\true);
        return new WP_REST_Response(['success' => \true]);
    }
    /**
     * See API docs.
