...
const Filter = (data) => {
    ...
    const applyFilters = ({ searchText, purchasability, metals_and_accessories, color, product_cat, onlineAvailable }) => {
                                ...
                                if (filterItem.type === "dropdown") {
                                    ...
                                    switch (filterName) {
                                        case "metals-and-accessories":
                                            stateKey = "metals_and_accessories";
...
<div className="container flex justify-center items-center my-24 flex-col gap-y-6">
 ...
 :
    !loading && displayedPosts.length < filteredPosts.length && (
        <button onClick={loadMorePosts} disabled={displayedPosts.length >= filteredPosts.length} className="inline-flex items-center gap-x-2.5">
