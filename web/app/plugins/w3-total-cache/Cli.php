...
class W3TotalCache_Command extends \WP_CLI_Command
{
    ...
    function fix_environment( $args = array(), $vars = array() )
    {
        ...
        try {
            ...
            \WP_CLI::error(
                __('Environment adjustment failed with error', 'w3-total-cache'),
    ...
    function pgcache_cleanup()
    {
        try {
            ...
            \WP_CLI::error(
                __(
                    'PageCache Garbage cleanup failed: %s',
                    'w3-total-cache' 
                ), $e 
            );
        }
        ...
        \WP_CLI::success(
            __(
                'PageCache Garbage cleanup triggered successfully.',
                'w3-total-cache' 
            ) 
        );
    }
    ...
    function pgcache_prime( $args = array(), $vars = array() )
    {
        try {
            ...
            \WP_CLI::error(
                __(
                    'PageCache Priming did failed: %s',
                    'w3-total-cache' 
                ), $e 
            );
        }
