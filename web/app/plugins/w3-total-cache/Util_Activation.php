...
class Util_Activation
{
    ...
    static public function throw_on_write_error( $path, $chmod_dirs = array() )
    {
        ...
        if (Util_File::check_open_basedir($path) ) {
            $error = sprintf(
                __(
                    '<strong>%s</strong> could not be created, please run following command:<br />%s',
                    'w3-total-cache' 
                ),
                $path, $chmods 
            );
            ...
            $error = sprintf(
                __(
                    '<strong>%s</strong> could not be created, <strong>open_basedir</strong> restriction in effect, please check your php.ini settings:<br /><strong style="color: #f00;">open_basedir = "%s"</strong>',
                    'w3-total-cache' 
                ),
                ...
                if (!$operation_error_already_shown ) {
                    ...
                    if ($ex instanceof Util_WpFile_FilesystemWriteException ) {
                        $required_changes .= sprintf(
                            __(
                                'Create the <strong>%s</strong> file and paste the following text into it: <textarea>%s</textarea> <br />',
                                'w3-total-cache' 
                            ),
        ...
        try {
            ...
            if (strlen($r['required_changes']) > 0 ) {
                ...
                $error = sprintf(
                    ...
                    __(
                        'or use FTP form to allow <strong>W3 Total Cache</strong> make it automatically.',
                        'w3-total-cache' 
                    ),
                    Util_Ui::button(
