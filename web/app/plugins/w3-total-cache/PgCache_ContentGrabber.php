...
class PgCache_ContentGrabber
{
    ...
    private function _can_write_cache( $buffer, $response_headers )
    {
        ...
        if (isset($response_headers['kv']['Location']) ) {
            ...
            if ($this->_get_page_key($this->_page_key_extension) ==        $this->_get_page_key($this->_page_key_extension, $response_headers['kv']['Location']) 
            ) {
    ...
    private function _get_key_extension()
    {
        ...
        $extension = w3tc_apply_filters(
            'pagecache_key_extension', $extension,
            $this->_request_url_fragments['host'], $this->_request_uri 
        );
    ...
    function _get_cached_headers( $response_headers )
    {
        ...
        $repeating_headers = apply_filters(
    ...
    function _parse_dynamic( $buffer )
    {
        ...
        $buffer = preg_replace_callback(
            '~<!--\s*mfunc\s*' . W3TC_DYNAMIC_SECURITY . '(.*)-->(.*)<!--\s*/mfunc\s*' . W3TC_DYNAMIC_SECURITY . '\s*-->~Uis', array(
            $this,
        ...
        $buffer = preg_replace_callback(
            '~<!--\s*mclude\s*' . W3TC_DYNAMIC_SECURITY . '(.*)-->(.*)<!--\s*/mclude\s*' . W3TC_DYNAMIC_SECURITY . '\s*-->~Uis', array(
            $this,
    ...
    private function _maybe_save_cached_result( $buffer, $response_headers, $has_dynamic )
    {
        ...
        foreach ( $compressions_to_store as $_compression ) {
            ...
            $_data = apply_filters(
                'w3tc_pagecache_set', $_data, $this->_page_key,
                $this->_page_group 
            );
        ...
        // Send headers
        $this->_send_headers(
            $is_404, $time, $etag, $compression_header,
            $headers 
        );
        return $buffer;
    }
