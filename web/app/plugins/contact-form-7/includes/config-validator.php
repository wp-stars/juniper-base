...
class WPCF7_ConfigValidator
{
    ...
    public function detect_unavailable_names( $section, $content )
    {
        ...
        $ng_named_tags = $form_tags_manager->filter(
            $content, array(
    ...
    {
        if ($this->contact_form->is_true('demo_mode') 
            or $this->contact_form->is_true('skip_mail')
        ) {
            return;
        }
