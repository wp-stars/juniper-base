...
class Extension_FragmentCache_Plugin_Admin
{
    ...
    {
        add_filter(
            'w3tc_objectcache_addin_required', array(
            $this, 'w3tc_objectcache_addin_required' ) 
        );

        add_action(
            'w3tc_environment_fix_on_event', array(
            '\W3TC\Extension_FragmentCache_Environment', 'fix_on_event' ),
            10, 2 
        );
        add_action(
            'w3tc_deactivate_extension_fragmentcache', array(
        ...
        add_filter(
            'w3tc_extension_plugin_links_fragmentcache',
            array( $this, 'w3tc_extension_plugin_links' ) 
        );
        add_action(
            'w3tc_settings_page-w3tc_fragmentcache',
            array( $this, 'w3tc_settings_page_w3tc_fragmentcache' ) 
        );

        add_action(
            'admin_init_w3tc_general', array(
    ...
    public function w3tc_admin_bar_menu( $menu_items )
    {
        if ($this->_config->is_extension_active_frontend('fragmentcache') ) {
            ...
            'href' => wp_nonce_url(
                admin_url(
                    'admin.php?page=w3tc_dashboard&amp;w3tc_flush_fragmentcache' 
                ), 'w3tc' 
            )
            );
        }
    ...
    public function w3tc_config_save( $config )
    {
        ...
        $config->set_extension_active_frontend(
            'fragmentcache',
            $is_frontend_active 
        );
    }
    ...
    public function w3tc_usage_statistics_summary_from_history( $summary, $history )
    {
        ...
        // counters
        $fragmentcache_calls_total = Util_UsageStatistics::sum(
            $history,
            'fragmentcache_calls_total' 
        );
        $fragmentcache_calls_hits = Util_UsageStatistics::sum(
            $history,
