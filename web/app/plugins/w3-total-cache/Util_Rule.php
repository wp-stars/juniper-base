        ...
        if (strpos($path, W3TC_CACHE_DIR) === false || Util_Environment::is_nginx() ) {
            ...
            try {
                ...
                if ($replace_start !== false ) {
                    $message = sprintf(
                        __(
                            'Edit file <strong>%s</strong> and replace all lines between and including <strong>%s</strong> and <strong>%s</strong> markers with:',
                            'w3-total-cache' 
                        ), $path, $start, $end 
                    );
                } else {
                    $message = sprintf(
                        __(
                            'Edit file <strong>%s</strong> and add the following rules above the WordPress directives:',
                            'w3-total-cache' 
                        ), $path 
                    );
                }
    ...
    static public function remove_rules( $exs, $path, $start, $end )
    {
        ...
        try {
            ...
            $exs->push(
				...
				and <strong>%s</strong> markers.', 'w3-total-cache' 
                        ), $path, $start, $end 
                    ), $path 
                ) 
            );
